  ...
  Widget productImage(Product p) {
    if (p.imageUrl.isEmpty) {
      return Container(
        ...
        child: const Icon(Icons.restaurant_menu, color: green, size: 34),
      );
    }
    return ClipRRect(
      ...
      child: Image.network(
        ...
        errorBuilder: (_, __, ___) => Container(
          ...
          height: 92,
          color: cream,
          child: const Icon(Icons.restaurant_menu, color: green),
        ),
      ),
    );
  }
  ...
  Widget logo() {
    if (widget.store.logoUrl.isNotEmpty) {
      return Image.network(
        ...
        height: 72,
        fit: BoxFit.contain,
        errorBuilder: (_, __, ___) => Image.asset(widget.store.localLogo, width: 72, height: 72, fit: BoxFit.contain),
      );
    }
                  ...
                  child: FilledButton(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        const Text('VER CARRINHO', style: TextStyle(fontWeight: FontWeight.w900)),
        ...
        body: FutureBuilder<List<Product>>(
          ...
          builder: (_, snap) {
            ...
            return ListView(
              ...
              children: [
                if (widget.store.coverUrl.isNotEmpty)
                  SizedBox(
                    height: 180,
                    width: double.infinity,
                    child: Image.network(widget.store.coverUrl, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const SizedBox.shrink()),
                  ),
                Padding(
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return Container(
                                ...
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Expanded(child: SizedBox(width: double.infinity, child: productImage(p))),
                                  const SizedBox(height: 6),
                                  Text(p.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900)),
                                  Text(money(p.price), style: const TextStyle(color: green, fontWeight: FontWeight.w900)),
                                ]),
                              );
                            },
                          ),
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (_, i) {
                            ...
                            return ChoiceChip(
                              ...
                              selectedColor: green,
                              labelStyle: TextStyle(color: selectedCategory == c ? Colors.white : Colors.black87, fontWeight: FontWeight.w800),
                            );
                          },
                        ),
                      ),
                      ...
                      ...filtered.map(
                        (p) => Container(
                          ...
                          child: Row(children: [
                            Expanded(
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                if (p.badge.isNotEmpty) Text(p.badge.toUpperCase(), style: const TextStyle(fontSize: 9, color: red, fontWeight: FontWeight.w900)),
                                Text(p.name, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900)),
                                const SizedBox(height: 5),
                                Text(p.description, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.black54)),
                                const SizedBox(height: 8),
                                Text(money(p.price), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: green)),
                              ]),
                            ),
                            ...
                            Stack(
                              ...
                              children: [
                                productImage(p),
                                IconButton.filled(onPressed: () => add(p), icon: const Icon(Icons.add)),
                              ],
                            ),
                          ]),
                        ),
