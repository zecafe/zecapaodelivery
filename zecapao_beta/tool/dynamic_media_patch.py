...
if 'Future<List<CategoryItem>> categories()' not in s:
    ...
    if marker not in s:
        raise SystemExit('core marker repo not found')
    s = s.replace(marker, add, 1)
      ...
      builder: (_, snap) {
        ...
        if (items.isEmpty) {
          return const SizedBox(
            height: 70,
            child: Center(child: Text('Categorias em atualização', style: TextStyle(color: brandMuted))),
          );
        }
        return SizedBox(
          height: 122,
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: Colors.white,
                          borderRadius: BorderRadius.circular(23),
                          boxShadow: const [BoxShadow(color: Color(0x18000000), blurRadius: 16, offset: Offset(0, 9))],
                        ),
    ...
    return FutureBuilder<List<CampaignBanner>>(
      ...
      builder: (_, snap) {
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (b.subtitle.isNotEmpty) ...[
                                ...
                                SizedBox(width: 250, child: Text(b.subtitle, maxLines: 2, style: const TextStyle(color: Colors.white70, fontSize: 11.5))),
                              ],
        ...
        if (zecafeStore != null && zecafeStore.coverUrl.isNotEmpty) {
          return InkWell(
            onTap: () => openStore(zecafeStore),
            borderRadius: BorderRadius.circular(29),
            child: ClipRRect(
              borderRadius: BorderRadius.circular(29),
              child: Image.network(
                zecafeStore.coverUrl,
                fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Container(
                  height: 204,
                  color: const Color(0xFF715441),
                  alignment: Alignment.center,
                  child: const Text('Zecafé', style: TextStyle(color: Colors.white, fontSize: 36, fontWeight: FontWeight.w900)),
                ),
              ),
            ),
          );
        ...
        return Container(
          height: 204,
          decoration: BoxDecoration(color: const Color(0xFF715441), borderRadius: BorderRadius.circular(29)),
          alignment: Alignment.center,
          child: const Text('Zecafé', style: TextStyle(color: Colors.white, fontSize: 36, fontWeight: FontWeight.w900)),
        );
      },
      ...
      builder: (_, snap) {
        ...
        if (items.isEmpty) {
          return Container(
            ...
            child: const Text('Agenda em atualização', style: TextStyle(color: brandMuted)),
          );
        }
        return SizedBox(
          height: 190,
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text(e.title, maxLines: 2, style: const TextStyle(color: Colors.white, fontSize: 25, height: 1.0, fontWeight: FontWeight.w900)),
                            if (e.subtitle.isNotEmpty) ...[
                              const SizedBox(height: 6),
                              Text(e.subtitle, maxLines: 2, style: const TextStyle(color: Colors.white70, fontSize: 11)),
                            ],
                            if (e.venue.isNotEmpty) ...[
                              const SizedBox(height: 6),
                              Text(e.venue, style: const TextStyle(color: brandYellow, fontSize: 9, fontWeight: FontWeight.w900)),
                            ],
                          ],
                        ),
                      ),
