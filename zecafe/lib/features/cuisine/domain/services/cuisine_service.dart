...
class CuisineService implements CuisineServiceInterface {
  ...
  @override
  Future<CuisineModel?> getCuisineList({DataSourceEnum? source, String? search}) async {
    return await cuisineRepositoryInterface.getList(source: source!, search: search);
  }
  ...
  Future<CuisineRestaurantModel?> getRestaurantList(int cuisineId, {String? name, FilterDataModel? filterDataModel}) async {
    return await cuisineRepositoryInterface.getRestaurantList(cuisineId, name: name, filterDataModel: filterDataModel);
  }
