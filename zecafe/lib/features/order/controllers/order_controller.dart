...
class OrderController extends GetxController implements GetxService {
  ...
  List<OrderModel>? _restaurantLastOrders;
  List<OrderModel>? get restaurantLastOrders => _restaurantLastOrders;
  ...
  List<OrderModel>? _homeLastOrders;
  List<OrderModel>? get homeLastOrders => _homeLastOrders;
  ...
  Future<void> reOrder(List<OrderDetailsModel> orderedFoods, int? restaurantZoneId) async {
    ...
    if(pir.$1 != -1){
      bool flg = false;
      await Get.dialog(ConfirmationDialogWidget(
        icon: Images.warning,
  ...
  Future<void> getRestaurantLastOrders(int restaurantId, {int offset = 1, bool notify = true}) async {
    if (offset == 1) {
    ...
    if (!_restaurantLastOrdersOffsetList.contains(offset)) {
      ...
      PaginatedOrderModel? result = await orderServiceInterface.getRestaurantLastOrders(restaurantId, offset: offset, limit: 5);
      if (result != null) {
  ...
  Future<void> getHomeLastOrders({int offset = 1, bool notify = true}) async {
    ...
    if (!_homeLastOrdersOffsetList.contains(offset)) {
      ...
      PaginatedOrderModel? result = await orderServiceInterface.getHomeLastOrders(offset: offset, limit: 5);
      if (result != null) {
  ...
  // Resets home and restaurant-wise last order state. Called on logout.
