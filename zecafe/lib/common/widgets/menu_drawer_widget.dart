...
class MenuDrawerWidgetState extends State<MenuDrawerWidget> with SingleTickerProviderStateMixin {
  final List<Menu> _menuList = [
    ...
    if(Get.find<SplashController>().proStaus) Menu(icon: Images.proPlanCrown, title: 'my_subscription'.tr, onTap: () {
      Get.back();
  ...
  void initState() {
    ...
    if(!Get.find<SplashController>().configModel!.proMemberStatus!) {
      _menuList.removeWhere((menu) => menu.title == 'my_subscription'.tr);
    }
