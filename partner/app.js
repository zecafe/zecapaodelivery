...
async function access(){
  ...
  if(profile?.role==='admin'){
    ...
    return {user,stores:data||[]};
  }
  const {data,error}=await sb.from('store_members').select('store_id,stores(*)').eq('user_id',user.id); if(error)throw error;
  return {user,stores:(data||[]).map(x=>x.stores).filter(Boolean)};
}

async function boot(){
  try{
    const a=await access();
    if(!a||!a.stores.length){showLogin(a?'Sua conta ainda não está vinculada a uma loja.':'');return}
...
function subscribeOrders(){
  ...
  channel=sb.channel(`partner-orders-${currentStore.id}`)
    .on('postgres_changes',{event:'*',schema:'public',table:'orders',filter:`store_id=eq.${currentStore.id}`},()=>loadOrders())
    .subscribe();
}
...
function renderOrders(){
  ...
  $('#orders').innerHTML=orders.map(o=>`<article class="order">
    ...
    <div class="meta">📍 ${esc(o.delivery_address||'Sem endereço')} ${o.notes?`<br>📝 ${esc(o.notes)}`:''}<br>💳 ${esc(o.payment_method||'')}</div>
...
window.changeStatus=async(id,status)=>{
  ...
  if(error)return toast('Não foi possível mudar o status');toast(`Pedido: ${statusLabels[status]}`);await loadOrders();
};

function renderProducts(){
  $('#products').innerHTML=`<table><thead><tr><th>Produto</th><th>Preço</th><th>Categoria</th><th>Status</th></tr></thead><tbody>${products.map(p=>`<tr><td><strong>${esc(p.name)}</strong><br><small>${esc(p.description||'')}</small></td><td>${money(p.price)}</td><td>${esc(p.category||'')}</td><td><button class="toggle ${p.is_available?'on':''}" onclick="toggleProduct('${p.id}',${!p.is_available})">${p.is_available?'Disponível':'Indisponível'}</button></td></tr>`).join('')}</tbody></table>`;
}
window.toggleProduct=async(id,value)=>{
  const {error}=await sb.from('products').update({is_available:value,updated_at:new Date().toISOString()}).eq('id',id);
  if(error)return toast('Erro ao alterar produto');toast(value?'Produto disponível':'Produto pausado');await loadProducts();
};
...
function renderStore(){
  ...
  const b=$('#toggleOpen');b.textContent=currentStore.is_open?'LOJA ABERTA':'LOJA FECHADA';b.className=`toggle ${currentStore.is_open?'on':''}`;
}
$('#toggleOpen').addEventListener('click',async()=>{
  const value=!currentStore.is_open;const {error}=await sb.from('stores').update({is_open:value,updated_at:new Date().toISOString()}).eq('id',currentStore.id);
  if(error)return toast('Erro ao alterar status da loja');currentStore.is_open=value;renderStore();toast(value?'Loja aberta':'Loja fechada');
});

const titles={orders:'Pedidos',products:'Produtos',store:'Minha loja'};
document.querySelectorAll('.nav').forEach(b=>b.addEventListener('click',()=>{
  document.querySelectorAll('.nav').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.view').forEach(v=>v.classList.add('hidden'));
  document.getElementById(`${b.dataset.view}View`).classList.remove('hidden');$('#title').textContent=titles[b.dataset.view];
}));
boot();